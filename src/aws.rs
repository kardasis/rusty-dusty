...
fn to_hash_attribute(distance_records: &DistanceRecordSet) -> HashMap<String, AttributeValue> {
    distance_records
        .iter()
...
impl Summary<'_> {
    fn attributes(&self) -> HashMap<&str, AttributeValue> {
        HashMap::from([
            ...
            (
                "bestDistances",
                M(to_hash_attribute(&self.distance_records)),
            ),
        ])
    }
}
...
pub async fn write_data_to_s3<T>(run_id: &str, data: T) -> Result<(), AwsError>
where
    T: Into<JsonValue>,
{
